...
`bundle_pre_1_25.yaml` ou `bundle_post_1_25.yaml` basé sur votre version de Kubernetes. Par exemple, si vous installez Trident 25.02, exécutez la commande suivante :
+
[listing]
----
...
.. Utilisez `helm upgrade` la commande où reflète la version vers laquelle `trident-operator-25.02.0.tgz` vous souhaitez effectuer la mise à niveau.
+
[listing]
----
