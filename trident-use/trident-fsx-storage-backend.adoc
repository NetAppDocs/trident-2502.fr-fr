...
| `securityStyle` | Style de sécurité pour les nouveaux volumes. Prise en charge NFS `mixed` et `unix` styles de sécurité. Prise en charge SMB `mixed` et `ntfs` styles de sécurité. | NFS la valeur par défaut est `unix`. SMB la valeur par défaut est `ntfs`. 
|===
